...
def get_scene_root_objects() -> Generator[bpy.types.Object, None, None]:
    ...
    for obj in bpy.context.scene.objects.values():
        if not obj.parent:
            # if named 'Empty', typed 'Empty', and dont have any children
            if obj.name == 'Empty' and obj.type == 'EMPTY' and len(obj.children) == 0:
                continue
            yield obj
...
def render_object(
    ...
    for i in range(num_renders):
        ...
        camera.data.angle = math.radians(fovs[i])

        x = distances[i] * math.cos(math.radians(elevations[i])) * math.cos(math.radians(azimuths[i]))
        y = distances[i] * math.cos(math.radians(elevations[i])) * math.sin(math.radians(azimuths[i]))
        z = distances[i] * math.sin(math.radians(elevations[i]))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=[1.5],
        nargs="+",
        help="Distance from the object, list of floats, same length as num_renders.",
    )
    parser.add_argument(
        "--elevation",
        default=[0],
    ...
    def repeat_value(list: List[float], length: int) -> List[float]:
        if len(list) == 1:
            return list * length
        return list

    args.distance = repeat_value(args.distance, args.num_renders)
    args.elevation = repeat_value(args.elevation, args.num_renders)
    args.azimuth = repeat_value(args.azimuth, args.num_renders)
    args.fov = repeat_value(args.fov, args.num_renders)
